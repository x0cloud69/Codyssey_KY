...
├── service.py        서비스 계층 — 업무 규칙, 집계, 예산 계산
...
### `budget_app` 하위 모듈별 책임

| 모듈 | 책임 | 직접 하지 않는 일 |
| --- | --- | --- |
| `__init__.py` | `budget_app` 폴더를 파이썬 패키지로 인식하게 한다 | 실행 흐름이나 업무 로직을 담지 않는다 |
| `__main__.py` | `python -m budget_app` 실행 시 `cli.main()`을 호출하는 진입점 역할을 한다 | 명령어 파싱, 저장, 계산을 직접 하지 않는다 |
| `cli.py` | `argparse`로 명령어와 옵션을 해석하고, 사용자 입력을 받고, 결과를 화면에 출력한다 | 파일을 직접 읽고 쓰거나 월별 합계를 직접 계산하지 않는다 |
| `service.py` | 거래 추가, 검색, 수정, 삭제, 요약, 예산, import/export 같은 업무 흐름을 조립한다 | JSONL 파일의 실제 읽기/쓰기 세부 구현을 직접 갖지 않는다 |
| `repository.py` | 거래, 카테고리, 예산 데이터를 JSONL 파일에 저장하고 읽는다. `stream()`으로 거래를 한 건씩 읽고, update/delete는 임시 파일 후 교체한다 | 사용자 입력을 받거나 화면 출력 형식을 정하지 않는다 |
| `models.py` | `Transaction` 데이터 구조를 정의하고 날짜, 금액, 타입, 카테고리 같은 기본 유효성을 검증한다 | 파일 저장 위치나 CLI 명령어를 알지 않는다 |
| `decorators.py` | 로그, 실행 시간 측정, 예외 처리처럼 여러 곳에서 반복될 수 있는 공통 처리를 함수 포장 형태로 제공한다 | 특정 명령의 업무 규칙을 직접 구현하지 않는다 |
| `config.py` | `data` 폴더, JSONL 파일 경로, 허용 거래 타입 같은 상수를 한곳에 모은다 | 실행 중 바뀌는 거래 데이터를 저장하지 않는다 |

### 왜 이렇게 나누었나

각 파일을 역할별로 나누면 한 부분을 고칠 때 다른 부분의 영향이 줄어든다. 예를 들어 화면 출력 문구를 바꾸고 싶으면 `cli.py`만 보면 되고, 저장 파일 이름을 바꾸고 싶으면 `config.py`를 보면 된다. 거래 데이터 검증은 `models.py`, 실제 파일 입출력은 `repository.py`, 여러 기능을 연결하는 업무 규칙은 `service.py`에 모여 있으므로 오류가 났을 때 원인을 찾기도 쉽다.

이 구조에서는 의존 방향도 단순하다. `cli.py`는 `BudgetService`를 호출하고, `service.py`는 저장소와 모델을 사용하며, `repository.py`는 파일과 모델만 다룬다. 그래서 CLI를 나중에 웹 화면이나 다른 입력 방식으로 바꾸더라도, `service.py`와 `repository.py`의 핵심 로직은 그대로 재사용할 수 있다.
...
service.py: 업무 규칙 처리
        ↓
repository.py: JSONL 파일 읽기/쓰기
        ↓
models.py: Transaction 생성 및 검증
```
